...
<template>
        ...
        <Column :exportable="false" style="min-width: 8rem" header="Actions">
          <template #body="slotProps">
            ...
            <Button
              ...
              rounded
              severity="danger"
              @click="confirmDeleteDomain(slotProps.data)"
            />
          </template>
        </Column>
        -->
  ...
  <DomainEntryCard
    v-if="editDomainDialog"
    :domain_data="domain_selected"
    :domain_state_dialog="editDomainDialog"
  ...
  <Dialog v-model:visible="deleteDomainDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
    <div class="flex items-center gap-4">
      ...
      <span v-if="domain_selected"
        >Are you sure you want to delete <b>{{ domain_selected.name }}</b
        >?</span
      >
    </div>
    <template #footer>
